...
contract PositionManager_decreasePosition_Test is Test {
    ...
    function test_decreasePosition() public {
        ...
        vm.prank(ALICE);
        positionManager.decreasePosition(CoreProxy, AccountProxy, ACCOUNT_ID, POOL_ID, CollateralToken_WETH, 5 ether);
        ...
        // Current liquidity position
        assertEq(45 ether, IVaultModule(CoreProxy).getPositionCollateral(ACCOUNT_ID, POOL_ID, CollateralToken_WETH));
        // Current available collateral
        assertEq(5 ether, ICollateralModule(CoreProxy).getAccountAvailableCollateral(ACCOUNT_ID, CollateralToken_WETH));
    }
}

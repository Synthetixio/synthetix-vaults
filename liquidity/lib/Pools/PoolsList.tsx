	...
	const poolsWithPositions = React.useMemo(() => {
		...
		return enrichedPools.map((pool) => {
			const position = positions.find(
				(position) =>
					position.collateralType.tokenAddress === pool.collateral.address,
			);
			return {
				...pool,
				rewardsValue:
					rewards?.find(
						(r) => r.collateralType.address === pool.collateral.address,
			...
			>
				...
				>
					...
					<Tooltip
						label={
							<Text textAlign="left">
								APR is averaged over the trailing 28 days and is comprised of
								both performance and rewards
							</Text>
						}
					>
				...
				>
					...
					<Tooltip
						label={
							<Text textAlign="left">
								Deposits can be withdrawn 24h after unlocking or any subsequent
								account activity
							</Text>
						}
					>
				...
				>
					...
					<Tooltip
						label={
							<Text textAlign="left">
								Unlocked assets can be locked into a position at any time or
								withdrawn after 24h since last activity
							</Text>
						}
					>
						<InfoIcon w="10px" h="10px" />

...
export async function clearDebt({
  ...
  const tx = await DebtRepayerContract.depositDebtToRepay(...args, { gasLimit: gasLimit.mul(2) });
  const result = await tx.wait();
  console.log('clearDebt', { txEvents: result.events.filter((e) => Boolean(e.event)) });
}
